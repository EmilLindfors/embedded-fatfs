...
async fn bench_allocation_at_fill_level(fill_pct: f32, num_allocations: u32) {
    println!("\n=== Benchmark: Allocation at {}% full ===", fill_pct * 100.0);
...
async fn main() {
    ...
    for name in &["bench_alloc", "bench_frag"] {
        ...
        match fs::copy("embedded-fatfs/resources/fat32.img", &target_path).await {
            Ok(_) => {},
            Err(_) => {
                fs::copy("resources/fat32.img", &target_path).await
    ...
    println!("  cargo bench --bench cluster_allocation --no-default-features --features std,alloc,lfn");
}
