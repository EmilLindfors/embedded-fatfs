...
async fn main() {
    ...
    // Copy test image
    match fs::copy("embedded-fatfs/resources/fat32.img", "target/bench_unbuffered.img").await {
        Ok(_) => {},
        Err(e) => {
            eprintln!("Failed to copy from embedded-fatfs/resources/fat32.img: {}", e);
            fs::copy("resources/fat32.img", "target/bench_unbuffered.img").await
...
async fn benchmark_unbuffered_sequential_read() {
    ...
    let fs = embedded_fatfs::FileSystem::new(io_counter.clone(), embedded_fatfs::FsOptions::new())
        .await
        .unwrap();
    ...
    println!("    Avg bytes/read: {:.0}", stats.bytes_read as f64 / stats.read_ops as f64);
    println!();
}
...
async fn benchmark_unbuffered_sequential_write() {
    ...
    println!("    Seeks: {}", stats.seek_ops);
    println!("    Total ops: {}", stats.total_ops());
    println!("    Avg bytes/write: {:.0}", stats.bytes_written as f64 / stats.write_ops.max(1) as f64);
    println!();
}
...
async fn benchmark_unbuffered_random_access() {
    ...
    println!("    Seeks: {}", stats.seek_ops);
    println!("    Total ops: {}", stats.total_ops());
    println!("    I/O ops per random access: {:.1}", stats.total_ops() as f64 / iterations as f64);
    println!();
}
