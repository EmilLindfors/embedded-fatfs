...
use std::fs::File;
use std::io::{Read as StdRead, Write as StdWrite, Seek as StdSeek};
use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::{Arc, Mutex};
use std::time::Instant;
use embedded_io_async::{Read, Write, Seek, SeekFrom, ErrorType};
...
fn setup_test_image() {
    ...
    // Copy test image
    if let Err(_) = std::fs::copy("embedded-fatfs/resources/fat32.img", "target/embassy_bench.img") {
...
async fn benchmark_sequential_read() {
    ...
    let fs = embedded_fatfs::FileSystem::new(device.clone(), embedded_fatfs::FsOptions::new())
        .await
        .unwrap();
...
async fn benchmark_random_access() {
    ...
    println!("    I/O ops per random read: {:.1}", stats.total_ops() as f64 / iterations as f64);
    println!();
    ...
    #[cfg(feature = "fat-cache")]
    {
        ...
        println!("    Misses: {}", cache_stats.misses);
        println!("    Hit rate: {:.1}%", cache_stats.hit_rate * 100.0);
        println!("    Cache effectiveness: Reduced FAT reads by {:.1}x",
...
fn print_sequential_results(operation: &str, bytes: u64, elapsed: std::time::Duration, stats: IoStatistics) {
    ...
    if operation == "READ" {
        ...
        println!("    Data read efficiency: {:.1}%",
        ...
        if stats.write_ops > 0 {
            println!("    Data write efficiency: {:.1}%",
