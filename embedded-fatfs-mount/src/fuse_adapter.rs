//! FUSE adapter for embedded-fatfs
//!
//! This module bridges FUSE operations to the embedded-fatfs library,
...
where
    IO: embedded_fatfs::ReadWriteSeek + Send,
    IO::Error: std::error::Error + 'static,
    TP: embedded_fatfs::TimeProvider + Send,
    OCC: embedded_fatfs::OemCpConverter + Send,
{
    ...
    fn lookup(&mut self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        let result = self.block_on(async {
            ...
            if parent == ROOT_INODE {
                ...
                let path_str = full_path.to_str().ok_or(embedded_fatfs::Error::InvalidInput)?;
    ...
    fn getattr(&mut self, _req: &Request, ino: u64, reply: ReplyAttr) {
        ...
        let result = self.block_on(async {
            let root = self.fs.root_dir();
            let path_str = path.to_str().ok_or(embedded_fatfs::Error::InvalidInput)?;
            root.open_meta(path_str.trim_start_matches('/')).await
        });
        ...
        let result = self.block_on(async {
            ...
            let dir = if ino == ROOT_INODE {
                ...
                let path_str = dir_path
                    .to_str()
                    .ok_or(embedded_fatfs::Error::InvalidInput)?;
            ...
            Ok::<Vec<_>, embedded_fatfs::Error<IO::Error>>(entries)
        });
    ...
    fn read(
        ...
        let result = self.block_on(async {
            ...
            Ok::<Vec<u8>, embedded_fatfs::Error<IO::Error>>(buffer)
        });
    ...
    fn create(
        ...
        let result = self.block_on(async {
            ...
            Ok::<_, embedded_fatfs::Error<IO::Error>>((file, entry))
        });
    ...
    fn mkdir(
        ...
        let result = self.block_on(async {
            ...
            Ok::<_, embedded_fatfs::Error<IO::Error>>(entry)
        });
    ...
    fn rename(
        ...
        let result = self.block_on(async {
            ...
            let old_path_str = old_path.to_str().ok_or(embedded_fatfs::Error::InvalidInput)?;
            let new_path_str = new_path.to_str().ok_or(embedded_fatfs::Error::InvalidInput)?;
    ...
    fn setattr(
        ...
        if let Some(new_size) = size {
            ...
            let result = self.block_on(async {
                let root = self.fs.root_dir();
                let path_str = file_path.to_str().ok_or(embedded_fatfs::Error::InvalidInput)?;
                root.open_meta(path_str.trim_start_matches('/')).await
            });
...
// Placeholder implementations for Windows (if winfsp support is added later)
#[cfg(windows)]
impl<IO, TP, OCC> Filesystem for FuseAdapter<IO, TP, OCC>
where
    IO: embedded_fatfs::ReadWriteSeek + Send,
