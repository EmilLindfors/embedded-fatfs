...
///
/// When the `send` feature is enabled, users should ensure their IO type
/// implements `Send` for use with multi-threaded executors like `tokio::spawn`.
/// Without `Send`, use `tokio::task::spawn_local` or single-threaded executors.
