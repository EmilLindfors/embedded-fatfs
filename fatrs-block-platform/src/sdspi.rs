...
/// SD cards use polynomial 0x09 with no reflection.
const CRC7_SD: crc::Algorithm<u8> = crc::Algorithm {
    width: 7,
    poly: 0x09,
    init: 0x00,
    refin: false,
...
/// CRC-7 calculator for SD card commands (minimal memory footprint).
const SD_CRC7: Crc<u8, NoTable> = Crc::<u8, NoTable>::new(&CRC7_SD);
...
impl<SPI, D, ALIGN, const SIZE: usize> fatrs_block_device::BlockDevice<SIZE>
...
/// The result includes the stop bit as required by SD protocol.
#[inline]
fn crc7(data: &[u8]) -> u8 {
    (SD_CRC7.checksum(data) << 1) | 1
}

/// Perform the X25 CRC-16 calculation, as used for data blocks.
#[inline]
fn crc16(data: &[u8]) -> u16 {
    SD_CRC16.checksum(data)
}
